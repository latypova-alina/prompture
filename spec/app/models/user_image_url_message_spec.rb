...
describe UserImageUrlMessage, type: :model do
  ...
  describe "delegations" do
    ...
    subject(:user_image_url_message) do
      create(
  ...
  describe "#resolved_image_url" do
    let(:user_image_url_message) { create(:user_image_url_message, image_url: "https://external.example/image.png") }

    context "when stored_image exists" do
      let!(:stored_image) do
        create(:stored_image, source_message: user_image_url_message, image_url: "https://internal.example/image.png")
      end

      it "returns stored_image url" do
        expect(user_image_url_message.resolved_image_url).to eq("https://internal.example/image.png")
      end
    end

    context "when stored_image does not exist" do
      it "returns original image_url" do
        expect(user_image_url_message.resolved_image_url).to eq("https://external.example/image.png")
      end
    end
  end
end
