...
describe UserPictureMessage, type: :model do
  ...
  describe "delegations" do
    ...
    subject(:user_picture_message) do
      create(
    ...
    it "delegates chat_id to command_request" do
      expect(user_picture_message.chat_id).to eq(123)
    end
  end

  describe "#resolved_image_url" do
    let(:user_picture_message) { create(:user_picture_message) }

    context "when stored_image exists" do
      let!(:stored_image) do
        create(:stored_image, source_message: user_picture_message, image_url: "https://internal.example/image.png")
      end

      it "returns stored_image url" do
        expect(user_picture_message.resolved_image_url).to eq("https://internal.example/image.png")
      end
    end

    context "when stored_image does not exist" do
      it "returns nil" do
        expect(user_picture_message.resolved_image_url).to be_nil
      end
    end
  end
end
