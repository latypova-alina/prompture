...
describe StarsPayment::CommandHandlerPresenter do
  ...
  describe "#formatted_text" do
    it { expect(presenter.formatted_text).to eq(I18n.t("telegram_webhooks.commands.buy_credits.ask", locale:)) }
  end
  ...
  describe "#inline_keyboard" do
    it "returns a single button opening the Mini App" do
      expect(presenter.inline_keyboard).to eq(
        [[
          {
            text: I18n.t("telegram_webhooks.commands.buy_credits.open_store_button", locale:),
            web_app: { url: "http://localhost:3000/mini_app/buy_credits" }
          }
        ]]
      )
    end
