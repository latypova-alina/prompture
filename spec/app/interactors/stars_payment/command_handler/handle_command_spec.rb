...
describe StarsPayment::CommandHandler::HandleCommand do
  ...
  let(:presenter) { instance_double(StarsPayment::CommandHandlerPresenter) }
  ...
  before do
    ...
    allow(StarsPayment::CommandHandlerPresenter)
      .to receive(:new)
      .with(locale:)
  ...
  describe "#call" do
    it "initializes presenter with locale" do
      ...
      expect(StarsPayment::CommandHandlerPresenter).to have_received(:new).with(locale:)
    end

    it "sends telegram message with presenter reply_data" do
      expect(bot).to receive(:send_message).with(chat_id:, **reply_data)
