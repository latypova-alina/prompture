...
describe ScriptGenerator::ForCats::ExtractTemplateName do
  describe ".call" do
    it "extracts template name from command text" do
      result = described_class.call(message_body: { "message" => { "text" => "/generate_cats_script daily_news" } })
    ...
    it "sets template_name to nil when argument is missing" do
      result = described_class.call(message_body: { "message" => { "text" => "/generate_cats_script" } })
