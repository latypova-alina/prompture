...
describe Generator::Media::Video::TaskRetrieverJob do
  ...
  let(:retriever_instance) { instance_double(Generator::Media::Video::RetrieveTask::TaskRetriever) }
  let(:media_url) { "http://example.com/video.mp4" }

  before do
    allow(
      Generator::Media::Video::RetrieveTask::TaskRetriever
    ).to receive(:new)
  ...
  describe "#perform" do
    context "when retriever succeeds" do
      before do
        ...
        allow(
          Generator::Media::Video::SuccessNotifierJob
