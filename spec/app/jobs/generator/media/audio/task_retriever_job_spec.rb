...
describe Generator::Media::Audio::TaskRetrieverJob do
  ...
  let(:retriever_instance) { instance_double(Generator::Media::Audio::RetrieveTask::TaskRetriever) }
  let(:stored_media) { instance_double(Generator::Media::StoredMedia::Retriever, internal_media_url: media_url) }
  let(:media_url) { "http://example.com/audio.mp3" }

  before do
    allow(
      Generator::Media::Audio::RetrieveTask::TaskRetriever
    ).to receive(:new)
  ...
  describe "#perform" do
    context "when retriever succeeds" do
      before do
        ...
        allow(
          Generator::Media::Audio::SuccessNotifierJob
      ...
      it "does not call ErrorNotifierJob" do
        expect(
          Generator::Media::Audio::ErrorNotifierJob
