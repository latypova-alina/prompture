...
describe ScriptGenerator::ProcessCartoonCharacterJob do
  describe "#perform" do
    before do
      allow(Telegram).to receive_message_chain(:bot, :send_message)
    end

    it "delegates to ScriptGenerator::ProcessCartoonCharacter service" do
      expect(ScriptGenerator::ProcessCartoonCharacter).to receive(:call).with(chat_id: 456)
    ...
    context "when script generator request fails" do
      before do
        allow(ScriptGenerator::ProcessCartoonCharacter).to receive(:call)
    ...
    context "when unexpected error occurs" do
      before do
        allow(ScriptGenerator::ProcessCartoonCharacter).to receive(:call).and_raise(StandardError, "boom")
      end
