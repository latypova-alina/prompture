...
describe ScriptGenerator::ForCats::GenerateScriptJob do
  describe "#perform" do
    before do
      allow(Telegram).to receive_message_chain(:bot, :send_message)
    end

    it "delegates to ScriptGenerator::ForCats::Processor service" do
      expect(ScriptGenerator::ForCats::Processor).to receive(:call).with(chat_id: 456, template_name: nil)
    ...
    context "when script generator request fails" do
      before do
        allow(ScriptGenerator::ForCats::Processor).to receive(:call)
    ...
    context "when unexpected error occurs" do
      before do
        allow(ScriptGenerator::ForCats::Processor).to receive(:call).and_raise(StandardError, "boom")
      end
