...
describe ScriptGenerator::ProcessScene::ForEditImage do
  subject(:service) { described_class.new(chat_id:, category:, reference_image_url:) }

  let(:chat_id) { 456 }
  let(:category) { ContentCategory::BLOOMY_CARTOON_SCRIPT }
  ...
  before do
    allow(ScriptGenerator::ProcessScene::StartEditImageGeneration).to receive(:call)
  end
  ...
  describe "#call" do
    ...
    it "starts edit image generation" do
      ...
      expect(ScriptGenerator::ProcessScene::StartEditImageGeneration)
