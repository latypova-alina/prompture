...
describe ScriptGenerator::ProcessScene::ForVideo do
  ...
  before do
    ...
    allow(SceneProcessor::ProcessScript).to receive(:call).and_return(result_context)
  end

  describe "#call" do
    it "delegates to SceneProcessor::ProcessScript with script, command_request, and chat_id" do
      service.call(script:)

      expect(SceneProcessor::ProcessScript).to have_received(:call).with(
        script:,
        command_request:,
        chat_id:,
        subcategory: nil
