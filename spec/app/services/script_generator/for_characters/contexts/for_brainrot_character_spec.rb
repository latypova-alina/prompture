...
describe ScriptGenerator::ForCharacters::Contexts::ForBrainrotCharacter do
  subject(:brainrot_character_context) { described_class.new(chat_id:) }
  ...
  let(:response) do
    instance_double(
      Faraday::Response,
      success?: true,
      body: { "character_description" => "A semi-realistic banana with crab claws" }
    )
  end

  before do
    allow(ScriptGenerator::Connection).to receive(:call).and_return(connection)
    allow(connection).to receive(:get).with("/brainrot_character").and_return(response)
  end

  describe "#character_description" do
    it "returns character description from response body" do
      expect(brainrot_character_context.character_description)
        .to eq("A semi-realistic banana with crab claws")
    end
    ...
    context "when Faraday raises an error" do
      before do
        allow(connection).to receive(:get).with("/brainrot_character")
