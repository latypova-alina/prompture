...
describe ScriptGenerator::ForBloomy::Payloads::ForSingleSceneScript do
  subject(:single_scene_script_payload) { described_class.new.payload }
  ...
  it "returns parsed response body" do
    expect(single_scene_script_payload).to eq(body)
  end
  ...
  context "when request fails with non-success status" do
    ...
    it "raises ScriptGeneratorRequestError" do
      expect { single_scene_script_payload }.to raise_error(ScriptGeneratorRequestError)
    end
  end
  ...
  context "when Faraday raises an error" do
    ...
    it "raises ScriptGeneratorRequestError" do
      expect { single_scene_script_payload }.to raise_error(ScriptGeneratorRequestError, "timeout")
    end
  end
end
