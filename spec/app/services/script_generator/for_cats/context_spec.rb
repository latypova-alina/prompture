...
describe ScriptGenerator::ForCats::Context do
  subject(:context) { described_class.new(chat_id:, template_name:) }
  ...
  describe "#script_array" do
    it "returns response body when request succeeds" do
      expect(context.script_array).to eq("scene one\n\nscene two")
    end
    ...
    context "when request fails with non-success status" do
      ...
      it "raises result error" do
        expect { context.script_array }.to raise_error(ScriptGeneratorRequestError)
      end
    end
