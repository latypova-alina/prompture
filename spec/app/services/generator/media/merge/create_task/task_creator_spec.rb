...
describe Generator::Media::Merge::CreateTask::TaskCreator do
  ...
  let(:strategy_selector_instance) { instance_double(Generator::Media::Merge::CreateTask::StrategySelector) }
  let(:strategy_instance) { instance_double("Strategy", api_url: api_url) }

  let(:payload_composer_instance) { instance_double(Generator::Media::Merge::CreateTask::PayloadComposer) }
  let(:final_payload) { { video_url: "https://example.com/video.mp4", audio_url: "https://example.com/audio.mp3" } }
  ...
  let(:api_url) { "https://queue.fal.run/fal-ai/ffmpeg-api/merge-audio-video" }
  let(:status) { 200 }

  before do
    allow(Generator::Media::Merge::CreateTask::StrategySelector)
      .to receive(:new)
      .with(request)
    ...
    allow(Generator::Media::Merge::CreateTask::PayloadComposer)
      .to receive(:new)
