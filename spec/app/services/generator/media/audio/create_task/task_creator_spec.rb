...
describe Generator::Media::Audio::CreateTask::TaskCreator do
  subject(:call_service) { described_class.call(request) }

  let(:request) { create(:button_audio_processing_request) }

  let(:strategy_selector_instance) { instance_double(Generator::Media::Audio::CreateTask::StrategySelector) }
  ...
  before do
    ...
    allow(Generator::Media::Audio::CreateTask::PayloadComposer)
      .to receive(:new)
